...
function renderExamList() {
    ...
    listDeThi.forEach(exam => {
        ...
        card.innerHTML = `
            ...
            <button type="button" class="bg-gray-50 text-apple-blue font-medium py-2.5 rounded-xl hover:bg-blue-50 transition w-full">
                Bắt đầu làm bài
            </button>
        `;
...
function submitExam() {
    ...
    currentExamData.forEach((q, index) => {
        ...
        if (userAnswer === q.correctAnswer) {
            score++;
            // Bôi xanh đáp án đúng mà user đã chọn
            correctLabel.classList.add('bg-green-50', 'border-green-400');
        } else {
            // Bôi đỏ đáp án user chọn sai (nếu có chọn)
            if (userAnswer) {
